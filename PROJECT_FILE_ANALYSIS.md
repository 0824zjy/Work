...
本文件基于当前工作区 `zjy_work` 的可读项目文件生成，重点分析源码、脚本、配置、说明和数据清单文件。

未展开逐条分析的数据包括：数据集图片、模型权重、训练日志、TensorBoard 事件、checkpoint、推理图片结果、缓存目录和训练输出目录。这些文件体量大且不属于可维护源码，文档中只在相关目录或脚本说明中提及其作用。
