...
class ControlNet(nn.Module):
    """
    ControlNet with mask as main condition and optional boundary prior as weak
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # ------------------------------------------------------------
        # Weak boundary prior branch
        # Compatible with old checkpoint key names.
        # Input can be hard boundary or soft boundary prior depending on
        # BOUNDARY_PRIOR_MODE, but it is always weakly scaled.
        # ------------------------------------------------------------
        ...
        **kwargs,
        ...
        for module, zero_conv in zip(self.input_blocks, self.zero_convs):
            if guided_hint is not None:
                ...
                # Safety alignment:
                # guided_hint comes from mask / boundary encoder.
...
class ControlLDM(LatentDiffusion):
    """
    Latent Diffusion with mask-conditioned ControlNet and optional boundary prior.

    Main path:
        mask / region condition.

    Boundary path:
        controlled by BOUNDARY_PRIOR_MODE:
            none: no boundary prior
            hard: hard morphological boundary
            soft: exponentially decayed soft boundary prior
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # New for multi-split controlled ablation:
        #   none -> mask-only control
        #   hard -> hard-boundary control
        #   soft -> soft prior / full SBG-Diff
        self.boundary_prior_mode = os.environ.get("BOUNDARY_PRIOR_MODE", "soft").lower()
        if self.boundary_prior_mode not in ["none", "hard", "soft"]:
            raise ValueError(
                f"Unsupported BOUNDARY_PRIOR_MODE={self.boundary_prior_mode}. "
                "Valid values are: none, hard, soft."
            )
    ...
    def _soft_boundary_prior_from_hard_boundary(self, hard_boundary: torch.Tensor) -> torch.Tensor:
        """
        Build a decayed soft band from an already extracted hard boundary.

        This avoids the old ambiguity where a hard boundary map was treated as a
        binary region mask and then edge-extracted again.
        """
        if hard_boundary is None:
            return None

        hard = _as_single_channel(hard_boundary.float())
        ...
        return soft.to(dtype=hard_boundary.dtype)
    ...
    def _soft_boundary_prior_from_mask(self, mask: torch.Tensor) -> torch.Tensor:
        ...
        return self._soft_boundary_prior_from_hard_boundary(hard)

    def _make_boundary_prior(
        self,
        x: torch.Tensor,
        input_is_boundary: bool = False,
    ) -> torch.Tensor:
        """
        Convert input mask/boundary to the selected boundary prior.

        BOUNDARY_PRIOR_MODE:
            none:
                return all-zero prior. This is used by mask-only ControlNet.

            hard:
                if input_is_boundary=True, use x as hard boundary directly.
                otherwise, extract hard boundary from mask.

            soft:
                if input_is_boundary=True, build decayed soft band around x.
                otherwise, extract hard boundary from mask, then build soft band.

        This is important for fair ablations:
            - hard-boundary should not be accidentally softened.
            - mask-only should not receive any boundary signal.
        """
        if x is None:
            return None

        x = _as_single_channel(x.float())
        x = torch.clamp(x, 0.0, 1.0)
        b, _, h, w = x.shape

        if self.boundary_prior_mode == "none":
            return torch.zeros(
                (b, 1, h, w),
                device=x.device,
                dtype=x.dtype,
        ...
        if self.boundary_prior_mode == "hard":
            if input_is_boundary:
                return (x > 0.5).float().to(dtype=x.dtype)
            return self._extract_hard_boundary_from_mask(x).to(dtype=x.dtype)

        if self.boundary_prior_mode == "soft":
            if input_is_boundary:
                return self._soft_boundary_prior_from_hard_boundary(x).to(dtype=x.dtype)
            return self._soft_boundary_prior_from_mask(x).to(dtype=x.dtype)

        raise NotImplementedError(self.boundary_prior_mode)
    ...
    def get_input(self, batch, k, bs=None, *args, **kwargs):
        ...
        # Explicit boundary_prior or boundary is treated as already boundary-like.
        # If neither exists, derive boundary from control mask.
        if "boundary_prior" in batch:
            boundary_prior = batch["boundary_prior"]

            if bs is not None:
                boundary_prior = boundary_prior[:bs]

            boundary_prior = boundary_prior.to(self.device)
            boundary_prior = _maybe_hwc_to_bchw(boundary_prior)
            boundary_prior = boundary_prior.to(memory_format=torch.contiguous_format).float()
            boundary_prior = torch.clamp(boundary_prior, 0.0, 1.0)

            boundary_prior = self._make_boundary_prior(
                boundary_prior,
                input_is_boundary=True,
            )

        elif self.boundary_key is not None and self.boundary_key in batch:
            boundary_prior = batch[self.boundary_key]
