...
class FeatureExtractor(nn.Module):
    ...
    This same structure is used for both region and boundary branches.
    """
...
class BoundarySpatialModulator(nn.Module):
    """
    Boundary-aware spatial modulation head.
    Input: 1-channel boundary map at current decoder scale
    Output: gamma/beta spatial maps, both 1-channel, broadcast across feature channels
    """
    def __init__(self, in_channels=1, hidden_channels=32, dim=2):
        super().__init__()
        self.net = nn.Sequential(
            conv_nd(dim, in_channels, hidden_channels, kernel_size=3, stride=1, padding=1),
            nn.SiLU(),
            conv_nd(dim, hidden_channels, hidden_channels, kernel_size=3, stride=1, padding=1),
            nn.SiLU(),
            conv_nd(dim, hidden_channels, 2, kernel_size=1, stride=1, padding=0),
        )
