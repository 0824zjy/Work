...
EXP_NAME="exp_mask2img_stage2_decoder_morph_adapt_aug"

STAGE1_EXP="exp_mask2img_stage1_cn_morph_adapt_only"
...
# ===== new method: adaptive consistency + morphology-balanced online aug =====
...
nohup python -u "${PROJECT_DIR}/tutorial_train.py" \
  > "${LOG_ROOT}/${EXP_NAME}/train_stage2_decoder_morph_adapt_aug_$(date +%Y%m%d_%H%M%S).log" 2>&1 &

echo "[OK] stage2 morph-adapt-aug started. resume=${STAGE1_CKPT}"
