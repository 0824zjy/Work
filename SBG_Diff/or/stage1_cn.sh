...
EXP_NAME="exp_mask2img_stage1_cn"
...
nohup python -u "${PROJECT_DIR}/tutorial_train.py" \
  > "${LOG_ROOT}/${EXP_NAME}/train_stage1_cn_$(date +%Y%m%d_%H%M%S).log" 2>&1 &

echo "[OK] stage1 started (CN-only). ckpt will be in ${LOG_ROOT}/${EXP_NAME}/checkpoints/"
