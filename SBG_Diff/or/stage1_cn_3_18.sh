# 修改 logger.py 中 root = os.path.join(save_dir, "image_log", split)
set -e

PROJECT_DIR="/data/zjy_work/BGDiff"
LOG_ROOT="/data/zjy_work/BGDiff/Ours/logs"
EXP_NAME="exp_mask2img_stage1_cn_taskaware"
...
nohup python -u "${PROJECT_DIR}/tutorial_train.py" \
  > "${LOG_ROOT}/${EXP_NAME}/train_stage1_cn_taskaware_$(date +%Y%m%d_%H%M%S).log" 2>&1 &

echo "[OK] stage1 task-aware started. ckpt will be in ${LOG_ROOT}/${EXP_NAME}/checkpoints/"
