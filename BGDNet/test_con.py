...
from models.MSDUnet import MSDUFormer_Xnet_3463_FUESMS_SwinConcat, MSDUFormer_Xnet_3463_FUESMS_CrossSwinConcat
from models.MSD import MSDUFormer_Xnet_3463_FUESMS_raw
...
if __name__ == '__main__':
    method_name = 'ISIC2018_FAUFormer_GlobalKV'
    ...
    # parser.add_argument('--pth_path', type=str, default='/zjy_work/MSDUNet-main/model_result/ISIC2018_FAUFormer_GlobalKV-raw-best.pth')
    # parser.add_argument('--pth_path', type=str, default='/zjy_work/MSDUNet-main/model_result/ISIC2018_FAUFormer_GlobalKV-swin-best.pth')
    parser.add_argument('--pth_path', type=str, default='/zjy_work/MSDUNet-main/model_out/ISIC2018-1(raw + dual + cross-attention )0.8979/ISIC2018_FAUFormer_GlobalKV/ISIC2018_FAUFormer_GlobalKV-best.pth')
    ...
    # ---- build models ----
    # torch.cuda.set_device(0)  # set your gpu device
    # model = MSDUFormer_Xnet_3463_FUESMS_raw(num_classes=args.num_classes).cuda(0)
    # model = MSDUFormer_Xnet_3463_FUESMS_SwinConcat(num_classes=args.num_classes).cuda(0)
    model = MSDUFormer_Xnet_3463_FUESMS_CrossSwinConcat(num_classes=args.num_classes).cuda(0)
    
    x = torch.load(args.pth_path)
    res = model.load_state_dict(x)
    ...
    for _data_name in ['test']:
        if dataset_name == 'HAM10000':
            ...
            data_path = '/zjy_work/ISIC2018/ISIC2018_Task1-2/{}'.format(_data_name)
            save_path = '/zjy_work/MSDUNet-main/model_result/stest4/'.format(_data_name)
        ...
        for i in range(num1):
            ...
            # 新增：收集每张图片的指标
            metrics_list.append({'image_name': name, 'dice': dice, 'jaccard': jacard})

        # 新增：保存每张图片的指标到 CSV
        df_metrics = pd.DataFrame(metrics_list)
        df_metrics.to_csv(save_path + 'per_image_metrics.csv', index=False)
